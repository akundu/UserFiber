...
size_t UFProducer::produceData(UFProducerData* ufpd, UF* uf)
{
    if(!uf)
        uf = UFScheduler::getUFScheduler()->getRunningFiberOnThisThread();
    _uf = (ufpd->_ufpcCode ? uf : 0);

    return updateConsumers(ufpd, uf);
}

inline void UFJoinableProducer::_updateConsumer(UFConsumer *ufc, UFProducerData *ufpd, UF *uf)
{
    ...
    if(!ufc->getNotifyOnExitOnly() || (ufpd->_ufpcCode == 0)) //signal/wake-up if necessary
    {
        if(_requireLockToUpdateConsumers)
            ufc->_queueOfDataToConsumeLock.signal();
        else
            UFScheduler::getUFScheduler()->addFiberToScheduler(ufc->getUF()); //wake up from a block
    }
...
size_t UFJoinableProducer::updateConsumers(UFProducerData* ufpd, UF* uf)
{
    ...
    {
        case 1: //optimize there being only one consumer; no need to iterate 
            _updateConsumer(_mostRecentConsumerAdded, ufpd, uf);
            break;
        default:
...
bool UFJoinableConsumer::removeProducer(UFJoinableProducer* ufp)
{
    ...
    return eraseProducer(ufp, _consumersProducerSet);
}

void UFProducer::resetProducer()
{
    UF* uf = UFScheduler::getUFScheduler()->getRunningFiberOnThisThread();
    while(_uf && _uf != uf) //potentially a different uf compared to the last uf that produced data is trying to delete the producer - we shouldnt allow that because the other uf might still think this producer is alive
        uf->usleep(1000000); //wait for the other uf (_uf) to finish producing
    _uf = 0;
