...
#include <UF/core/UFServer.H>
#include <UF/core/UFConnectionPool.H>
#include <vector>
...
void printResults()
{
    ...
    int counter = 0;
    cerr<<"percentile breakdown w/ size = "<<overallInfo.results.size()<<endl;
         ...
         ++it)
    {
        ...
        {
            cerr<<" "<<currLocation<<"%"<<" <= "<< *it<<"us"<<endl;
            lastDump = currLocation;
        }
        lastValue = *it;
    }
    cerr<<"100%"<<" <= "<< lastValue <<"us"<<endl;
    cerr<<"min = "<<*(overallInfo.results.begin())<<"us"<<endl;
    cerr<<"max = "<<lastValue<<"us"<<endl;
}
